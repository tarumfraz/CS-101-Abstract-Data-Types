...
class BSTree(object): # Create a class for the actual BST implementation
	...
	def successor(self, key):
		...
		if key ==  None: # If input it is empty or not in BST
			return None # Return Null
		if key.right is not None: # If the value has a right child
			successor = key.right # The sucessor is the right child
		...
		if key.right is None: # If the val has no right child
			successor = key.parent.right # Return the right most ancestor of the value
			return successor
...
print(lowRating.inOrderTraversal())
